...
# --- CONFIGURATION ---
# Use environment variable for security, fallback to default for local dev
...
if DATABASE_URL:
    # Production: Use PostgreSQL (Railway uses postgres://, SQLAlchemy needs postgresql://)
...
db.init_app(app)
migrate = Migrate(app, db)  # Added Flask-Migrate for database schema management
...
def signup():
    if request.method == 'POST':
        ...
        # ✅ FIX: Check for BOTH phone and email to prevent database crashes
...
def login():
    if request.method == 'POST':
        ...
        print(f"Identifier received: '{login_identifier}'")
        print(f"Password received: '{password}'")
        
        user = User.query.filter(
            or_(User.phone == login_identifier, User.email == login_identifier)
        ).first()
...
def api_get_contracts():
    ...
    for c in contracts:
        result.append({
            ...
            'projectLocation': c.project_location,
            'projectTimeline': c.project_timeline,
            'projectDescription': c.project_description,
            'buildingPlan': c.building_plan,
            'status': c.status,
            'signature': c.signature,
            'signatureMethod': c.signature_method,
...
def api_get_technician_profile():
    ...
    return jsonify({
        ...
        'isActive': current_user.is_active_account,
        'availabilityStatus': current_user.availability_status or 'available',
...
def api_upload_gallery():
    ...
    for file in files:
        if file and file.filename != '' and allowed_file(file.filename):
            ...
            new_image = TechnicianGallery(
                user_id=current_user.id, 
                image_path=f"/static/uploads/gallery/{unique_filename}"
            )
...
def api_submit_contract():
    try:
        ...
        if not all([client_name, client_email, client_phone, client_country, 
                   project_type, project_location, project_timeline]):
            return jsonify({
                'success': False, 
        ...
        new_contract = Contract(
            ...
            building_plan=building_plan_path,
            status='pending'
        )
...
def api_get_contract(token):
    ...
    return jsonify({
        ...
        'clientCountry': contract.client_country,
        'projectType': contract.project_type,
        'projectLocation': contract.project_location,
        'projectTimeline': contract.project_timeline,
        'projectDescription': contract.project_description,
        'buildingPlan': contract.building_plan,
        'status': contract.status,
        'signature': contract.signature,
        'signatureMethod': contract.signature_method,
...
def api_sign_contract(token):
    ...
    if contract.client_email:
        html_content = f"""
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
            <h2 style="color: #2563EB;">Contract Signed Successfully! ✍️</h2>
            ...
            <p style="margin-top: 20px;">Contract Reference: <strong>{contract.token}</strong></p>
            <p style="margin-top: 30px; font-size: 12px; color: #64748b;">Best regards,<br>The TECHNISERVE Team</p>
        </div>
        """
...
# ==========================================
# --- SEARCH API ROUTE (OPTIMIZED & FIXED) ---
# ==========================================
...
def api_search_technicians():
    ...
    # ✅ FIX: Use EXACT match for the skill dropdown to prevent "Electrical" from matching "Electrical & Solar"
    if skill:
        q = q.filter(User.skill == skill)

    # Filter by Location (SKIP for Automation searches)
    if location and not is_automation_search:
        search_term = f"%{location}%"
        q = q.filter(User.location.ilike(search_term))

    # General text search (Name, Skill, Location) - keep ilike here for flexible typing in the search bar
    if query:
        search_term = f"%{query}%"
        q = q.filter(or_(
            User.full_name.ilike(search_term),
            User.skill.ilike(search_term),
    ...
    for t in technicians:
        result.append({
            ...
            'profileUrl': url_for('technician_profile', phone=t.phone),
            'isAutomation': is_automation_search,
            'availabilityStatus': t.availability_status or 'available'
        })

    return jsonify({
        'technicians': result,
        'isAutomationSearch': is_automation_search,
        'totalResults': len(result)
    }), 200

# ==========================================
# --- PUBLIC TECHNICIAN PROFILE API (OPTIMIZED) ---
# ==========================================
...
def api_get_public_technician():
    ...
    return jsonify({
        ...
        'profilePic': tech.profile_pic,
        'registeredAt': tech.created_at.isoformat() if tech.created_at else None,
        'gallery': gallery_urls,
...
def api_get_technician_jobs():
    ...
    print(f"\n🔍 DEBUG: Fetching work for Tech ID: {tech.id}")
    
    # 1. Fetch Posted Jobs
    jobs = Job.query.filter(
        or_(
            (Job.technician_id == tech.id) & (Job.status.in_(['active', 'completed'])),
            (Job.status == 'new') & (Job.skill_required.ilike(f"%{tech.skill}%"))
        )
    ).order_by(Job.created_at.desc()).all()
    
    # 2. Fetch Direct Bookings (✅ EXCLUDE SILENT CONTACT LOGS)
    bookings = Booking.query.filter(
        Booking.technician_id == tech.id,
        Booking.source.notin_(['direct_call', 'direct_whatsapp'])
    ...
    for j in jobs:
        ...
        result.append({
            ...
            'location': j.location,
            'budget': j.budget or 'Negotiable',
            'description': j.description,
            'status': j.status,
            'createdAt': j.created_at.isoformat()
        })
        
    # Format Direct Bookings to look like Jobs
    for b in bookings:
        result.append({
            'id': b.id,
            'item_type': 'booking',
            ...
            'location': 'Direct Profile Booking',
            'budget': 'Direct Booking',
            'description': b.notes or 'Client booked this service directly from your public profile.',
            'status': b.status, 
            'createdAt': b.booking_date.isoformat()
        })
...
def api_accept_item(item_id):
    # First, check if it's a direct booking
    ...
    # Otherwise, treat it as a posted job
    job = Job.query.get_or_404(item_id)
    
    # ✅ FIX: Allow if already assigned, OR if it's a 'new' job matching their skill
    ...
    job.status = 'active'
    job.technician_id = current_user.id  # ✅ Officially claim the job!
    db.session.commit()
...
def api_create_booking():
    ...
    if tech.email:
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
            ...
            <div style="background: #f3f4f6; padding: 16px; border-radius: 8px; margin: 16px 0;">
                <h3 style="margin: 0 0 12px 0; color: #111;">Booking Details</h3>
                <p style="margin: 8px 0; color: #64748b;"><strong>Service:</strong> {service_type}</p>
                <p style="margin: 8px 0; color: #64748b;"><strong>Client:</strong> {client_name}</p>
                {f'<p style="margin: 8px 0; color: #64748b;"><strong>Notes:</strong> {notes}</p>' if notes else ''}
            </div>
            <p>Please log in to your dashboard to accept this booking and contact the client.</p>
            <a href="{url_for('login', _external=True)}" style="display: inline-block; padding: 12px 24px; background-color: #2563EB; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; margin-top: 16px;">View Booking in Dashboard</a>
            <p style="margin-top: 30px; font-size: 12px; color: #64748b;">Best regards,<br>The TECHNISERVE Team</p>
        </div>
        """
    ...
    return jsonify({
        'success': True, 
        'message': 'Booking request sent to technician!',
        'bookingId': new_booking.id
    }), 201

# ==========================================
# --- POST A JOB ROUTES (FLEXIBLE FOR GUESTS & CLIENTS) ---
# ==========================================
...
def api_create_public_job():
    ...
    new_job = Job(
        ...
        location=location,
        budget=budget if budget else 'Negotiable',
        client_name=client_name,
        client_phone=client_phone,
        status='new'
    )
    ...
    matching_techs = User.query.filter_by(
        role='technician', 
        ...
        if tech.email:
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
                ...
                <div style="background: #f3f4f6; padding: 16px; border-radius: 8px; margin: 16px 0;">
                    ...
                    <p style="margin: 4px 0; color: #64748b;"><strong>Location:</strong> {location}</p>
                    <p style="margin: 4px 0; color: #64748b;"><strong>Budget:</strong> {budget or 'Negotiable'}</p>
                </div>
    ...
    return jsonify({
        'success': True, 
        'message': 'Job posted successfully! Technicians in your area will be notified.',
        'jobToken': job_token,
        'matchingTechnicians': len(matching_techs)
    }), 201
...
# ==========================================
# --- AUTH & OTP ROUTES (FIXED TIMEZONE BUG) ---
# ==========================================
...
def api_forgot_password():
    ...
    html_content = f"""
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
        <h2 style="color: #2563EB;">TECHNISERVE Password Reset</h2>
        <p>Hello,</p>
        <p>Your verification code is:</p>
        <h1 style="background: #f3f4f6; padding: 15px; text-align: center; letter-spacing: 5px; color: #111;">{otp_code}</h1>
...
def api_admin_analytics_popular_skills():
    skills = db.session.query(User.skill, db.func.count(User.id).label('count')).filter(
        User.role == 'technician', User.status == 'approved'
    ).group_by(User.skill).order_by(db.desc('count')).limit(10).all()
...
def api_admin_analytics_technician_performance():
    ...
    for tech in technicians:
        ...
        result.append({
            'id': tech.id, 'name': tech.full_name, 'skill': tech.skill, 'location': tech.location,
            'totalBookings': total_work, 'completedBookings': completed_work, 'monthlyBookings': monthly_work, 'profilePic': tech.profile_pic
        })
...
def api_admin_analytics_booking_source():
    # ✅ UPDATED: Count ALL 4 contact methods by source
    ...
    return jsonify({
        'jobBoard': job_board_count,
        'directBooking': direct_booking_count,
        'directCall': direct_call_count,
        'directWhatsapp': direct_whatsapp_count
    }), 200

# ==========================================
# --- SILENT CONTACT LOGGING (Anti-Bypass) ---
# ==========================================

@app.route('/api/technician/log-contact', methods=['POST'])
def api_log_contact():
    """Silently logs when a client clicks Call or WhatsApp on a technician's profile."""
    data = request.get_json()
    technician_phone = data.get('technicianPhone', '').strip()
    ...
    new_log = Booking(
        technician_id=tech.id,
        client_name='Anonymous Visitor',
        client_phone='N/A',
        service_type='Profile Contact',
        source=contact_method,
        status='completed',
        booking_date=datetime.now(timezone.utc),
        notes=f'Client contacted technician via {"phone call" if contact_method == "direct_call" else "WhatsApp"} from profile page.'
    )
